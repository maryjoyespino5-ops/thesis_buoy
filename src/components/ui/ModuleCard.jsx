...
export function ModuleCard({ icon, title, status, tag, onClick }) {
  return (
    <motion.div
      ...
      <h4 className="text-sm font-semibold text-text-primary mb-0.5 leading-tight">
        {title}
      </h4>
      <p className="text-sm text-text-secondary">{status}</p>
      {tag && (
        <span className="inline-block mt-1.5 text-xs font-medium text-primary-500 bg-primary-50 px-1.5 py-0.5 rounded">
          {tag}
        </span>
      )}
    </motion.div>
